...
data_dir =  '/pfs/work7/workspace/scratch/ma_luitheob-tp2021/turl/turl-base/medium/eval_input/'
...
with open(os.path.join(data_dir, 'new_test_representation.txt'), 'rb') as f:
    example = pickle.load(f)[0]
display(example)

# load type vocab from type_vocab.txt
type_vocab = load_type_vocab(data_dir)
test_dataset = WikiCTDataset(data_dir, entity_vocab, type_vocab, max_input_tok=500, src="new_test", max_length = [50, 10, 10], force_new=False, tokenizer = None)
len(type_vocab)
...
from tqdm.autonotebook import tqdm
checkpoints = [
    "medium/output/mode_0/pytorch_model.bin",
    "medium/output/mode_1/pytorch_model.bin",
    "medium/output/mode_2/pytorch_model.bin",
    "medium/output/mode_3/pytorch_model.bin",
    "medium/output/mode_4/pytorch_model.bin"
]
